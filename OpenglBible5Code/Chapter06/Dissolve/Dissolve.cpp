...
// 本例子主要是讲述GLSL discard函数，通过之前的案例我们可以知道，普通的颜色混合操作需要从颜色缓冲区进行一次读取，
// 至少两次乘法和对颜色的一次求和，然后再将得到的颜色值写回到颜色缓冲区。如果得到的颜色alpha为0，获得接近0，那么这个
// 片段实际上是不可见的，绘制不可见的东西实在是毋庸的消耗性能，而且在深度缓冲区中创建一个不可见的片段也会导致深度测试
// 的异常。所以针对这样的情况，我们可以设置alpha小于某一个阈值的时候直接丢弃渲染这个片段。
...
void SetupRC(void)
	...
	ADSDissloveShader = gltLoadShaderPairWithAttributes("Chapter06/Dissolve/Dissolve.vp", "Chapter06/Dissolve/Dissolve.fp", 3, GLT_ATTRIBUTE_VERTEX, "vVertex",
	...
	LoadTGATexture("Chapter06/Dissolve/Clouds.tga", GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR, GL_CLAMP_TO_EDGE);
	}
