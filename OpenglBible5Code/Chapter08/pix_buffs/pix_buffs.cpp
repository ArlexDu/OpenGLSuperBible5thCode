//从这个案例中可以看出,缓冲区可以摆脱强类型对象的束缚，可以方便的从一个渲染管线移动到另外一个渲染管线
//比如在本例子中，当保存当前画面的时候，将缓冲区绑定到GL_PACK_PIXEL_BUFFER，当绑定纹理对象的时候
//将缓冲区绑定为GL_UNPACK_PIXEL_BUFFER;
#include <stdio.h>
#include <iostream>
...
void SetupRC(void)
{
	...
	LoadBMPTexture("Chapter08/pix_buffs/marble.bmp", GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR, GL_REPEAT);

	// Create blur program
	blurProg =  gltLoadShaderPairWithAttributes("Chapter08/pix_buffs/blur.vs", "Chapter08/pix_buffs/blur.fs", 2,
	...
	//设置glReadPixels之类的像素包装操作的目标缓冲区，即设置glReadPixels想像素数据复制到该缓冲区中
	glBindBuffer(GL_PIXEL_PACK_BUFFER, pixBuffObjs[0]);
	//填充缓冲区，第一个参数与绑定相同，第二个参数是我们要上传数据的大小（以字节为单位），第三个参数是将要被上传的数据本身
	//如果想分配一个特定大小的缓冲区却不立即对它进行填充，那么这个指针可以设置为NULL,第四个参数是告诉OpenGL如何使用这个参数，实际上是一个性能的提示
	//GL_DYNAMIC_COPY表示缓冲区的内容作为OpenGL的命令输出经常更新，并且经常用于绘制或复制到其他的图像
...
void RenderScene(void)
{
	...
	if(bUsePBOPath)//新的PBO模式，获取当前画面的数据不再传回给CPU,而是将PBO绑定到GL_PIXEL_PACK_BUFFER,而在调用glReadPixels的时候
		//这些像素被重新定向到PBO而不是再次回到CPU，然后该缓冲区将从GL_PIEXL_PACK_BUFFER的绑定点张解除绑定，然后再绑定到
		//GL_PIXEL_UNPACK_BUFFER。则接下来调用glTexImage2D的时候，缓冲区的数据将被加载到纹理上，这些数据一直没有离开GPU
		//也不会和其他命令一起保持在管线中，从而节省了从CPU到GPU的移动数据过程。
	{
	...
	else//传统模糊方法：通过调用glReadPixels获取像素数据，然后调用glTexImage2D将像素数据复制到一个纹理对象上完成，数据的纹理目标
		//在6个模糊纹理之间进行轮换。如果上一次使用的是纹理3，则下一个使用的将是纹理4。纹理4中包含来自这一帧的数据，纹理3包含前一帧
		//的数据，而纹理2则包含再前一帧的数据，以此类推
	{
