...
void RenderScene(void)
	...
	// 设置一个计时器，从0到28循环，表示当前使用的哪一张切图
	float fTime = timer.GetElapsedSeconds();
	fTime = fmod(fTime, 28.0f);
	printf("fTime is %f\n",fTime);
    glUniform1f(locMoonTime, fTime);
...
void SetupRC()
    ...
    starFieldShader = gltLoadShaderPairWithAttributes("Chapter07/TextureArrays/StarField.vp", "Chapter07/TextureArrays/StarField.fp", 1, GLT_ATTRIBUTE_VERTEX, "vVertex");

	locMVP = glGetUniformLocation(starFieldShader, "mvpMatrix");
	locStarTexture = glGetUniformLocation(starFieldShader, "starImage");

	moonShader = gltLoadShaderPairWithAttributes("Chapter07/TextureArrays/MoonShader.vp", "Chapter07/TextureArrays/MoonShader.fp", 2, GLT_ATTRIBUTE_VERTEX, "vVertex",
	...
	glBindTexture(GL_TEXTURE_2D, starTexture);
	LoadTGATexture("Chapter07/TextureArrays/Star.tga", GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR, GL_CLAMP_TO_EDGE);
	...
	//最后一个参数设置为NULL,表示没有数据复制，但是OpenGL保留了纹理的存储空间
	glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_RGBA, 64, 64, 30, 0,
					 GL_BGRA, GL_UNSIGNED_BYTE, NULL);

	for(int i = 0; i < 29; i++) {
		//选择一个合适的字符串存储空间，不然运行时会导致buffer too small错误
		char cFile[128];
		sprintf_s(cFile, "Chapter07/TextureArrays/moon%02d.tga", i);
		printf("load %s\n", cFile);
		GLbyte *pBits;
		int nWidth, nHeight, nComponents;
		GLenum eFormat;
