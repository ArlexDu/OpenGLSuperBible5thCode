...
// 载入矩阵纹理，因为矩阵纹理不支持Mip所以就不用加Mipmap的参数判断
...
void SetupRC()
	...
	LoadTGATexture("Chapter07/TextureRect/marble.tga", GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR, GL_REPEAT);
	
	// Load Mars
	glBindTexture(GL_TEXTURE_2D, uiTextures[1]);
	LoadTGATexture("Chapter07/TextureRect/marslike.tga", GL_LINEAR_MIPMAP_LINEAR, 
				   GL_LINEAR, GL_CLAMP_TO_EDGE);
	
	// Load Moon
	glBindTexture(GL_TEXTURE_2D, uiTextures[2]);
	LoadTGATexture("Chapter07/TextureRect/moonlike.tga", GL_LINEAR_MIPMAP_LINEAR,
				   GL_LINEAR, GL_CLAMP_TO_EDGE);

	// Load the Logo
	glBindTexture(GL_TEXTURE_RECTANGLE, uiTextures[3]);
	LoadTGATextureRect("Chapter07/TextureRect/OpenGL-Logo.tga", GL_NEAREST, GL_NEAREST, GL_CLAMP_TO_EDGE);

    rectReplaceShader = gltLoadShaderPairWithAttributes("Chapter07/TextureRect/RectReplace.vp", "Chapter07/TextureRect/RectReplace.fp", 
...
void RenderScene(void)
	...
	// 翻转之后，几何物体的构建方式从逆时针变成顺时针了，所以这里要设置顺时针为正面，因为绘制的时候,开启了背面剔除
	glFrontFace(GL_CW);
	...
	// 这里开启混合是为了场景翻转影像和地面融合，展现出大理石倒影的效果
	glEnable(GL_BLEND);
	glBindTexture(GL_TEXTURE_2D, uiTextures[0]);
    ...
    // 这里绘制的屏幕右下角的OpenGL的标志，在屏幕中间进行2D绘制时，普遍的做法是创建一个与屏幕大小相匹配的正投影矩阵
	// 我们选择让坐标系与屏幕上的纹理匹配，但是将原点（0,0）设置在了左下角
    M3DMatrix44f mScreenSpace;
	// m3dMakeOrthographicMatrix(Xmin,Xmax,Ymin,Ymax,Zmin,Zmax)
    m3dMakeOrthographicMatrix(mScreenSpace, 0.0f, 800.0f, 0.0f, 600.0f, -1.0f, 1.0f);
        
    // 因为这里是绘制浮于屏幕表面的2维贴图，所以其和场景本身并无关系，不需要进行深度测试
	glEnable(GL_BLEND);
